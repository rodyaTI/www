...
class EAuth extends CApplicationComponent {
	...
	public function init() {
		...
		if (SocialauthModel::getSocialParamValue('useGoogleOauth')) {
			$this->services['google_oauth'] = array(
					// register application: https://code.google.com/apis/console/
					'class' => 'CustomGoogleService',
					'client_id' => SocialauthModel::getSocialParamValue('googleOauthClientId'),
					'client_secret' => SocialauthModel::getSocialParamValue('googleOauthClientSecret'),
			);
		}
		if (SocialauthModel::getSocialParamValue('useTwitter')) {
			$this->services['twitter'] = array(
					// register application: https://dev.twitter.com/apps/new
					'class' => 'CustomTwitterService',
					'key' => SocialauthModel::getSocialParamValue('twitterKey'),
					'secret' => SocialauthModel::getSocialParamValue('twitterSecret'),
			);
		}
		if (SocialauthModel::getSocialParamValue('useFacebook')) {
			$this->services['facebook'] = array(
					// register application: https://developers.facebook.com/apps/
					'class' => 'CustomFBService',
					'client_id' => SocialauthModel::getSocialParamValue('facebookClientId'),
					'client_secret' => SocialauthModel::getSocialParamValue('facebookClientSecret'),
			);
		}
		if (SocialauthModel::getSocialParamValue('useVkontakte')) {
			$this->services['vkontakte'] = array(
					// register application: http://vk.com/editapp?act=create&site=1
					'class' => 'CustomVKService',
					'client_id' => SocialauthModel::getSocialParamValue('vkontakteClientId'),
					'client_secret' => SocialauthModel::getSocialParamValue('vkontakteClientSecret'),
			);
		}
	}
	...
	protected function getService($service) {
		...
		if (!isset($services[$service]))
			throw new EAuthException(Yii::t('module_socialauth', 'Undefined service name: {service}.', array('{service}' => $service)), 500);
