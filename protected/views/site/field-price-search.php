		...
		<span class="search">
			...
			if (is_array($apTypes) && count($apTypes['propertyType'] > 0)) {
				...
				foreach ($apTypes['propertyType'] as $key => $value) {
					...
					echo '<div style="display: none;" id="price-currency-'.$key.'" class="slider-price-currency">'.param('siteCurrency', 'руб.').'</div>';
			...
			?>
			<span class="search"><div class="<?php echo $textClass; ?>" id="currency-title"><?php echo tc('Price to'); ?>:</div> </span>
			<span class="search">
				<input type="text" id="priceTo" name="price" class="width70 search-input-new" value="<?php echo isset($this->price) && $this->price ? $this->price : ""; ?>"/>&nbsp;
				<span id="price-currency"><?php echo param('siteCurrency', 'руб.'); ?></span>
			</span>
	<?php
