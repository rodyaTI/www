	...
	echo '<div class="similar-ads">';
		...
		echo '<ul id="mycarousel" class="jcarousel-skin-tango">';
			foreach ($ads as $item) {
				...
				echo '<li>';
					echo '<a href="'.Yii::app()->createUrl('apartments/main/view', array('id' => $item->id/*, 'title' => $item->{'title_'.Yii::app()->language}*/)).'">';
						if ($image) {
							echo '<img src="' . Yii::app()->request->baseUrl . '/uploads/apartments/' . $item->id . '/mediumthumbs/' . $image . '" alt="" title="'.$item->{'title_'.Yii::app()->language}.'" width="150" height="100" />';
						}
						else {
							echo '<img src="' . $imageDefault . '" alt="" title="'.$item->{'title_'.Yii::app()->language}.'" width="150" height="100" />';
						}
					echo '</a>';
